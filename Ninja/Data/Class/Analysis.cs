    ...
    namespace Ninja
    {
        namespace Data
        {
            public class Analysis
            {
                ...
                public DivisionAuthority DivisionArray { get; }
                public Dictionary<string, decimal[]> P8Info { get; }
                public DivisionAuthority[] P8 { get; }
                ...
                public Analysis(string rc)
                {
                    EPA = new RegionalAuthority( );
                    DivisionArray = new DivisionAuthority(rc);
                }

                public Analysis(DivisionAuthority division)
                {
                    EPA = new RegionalAuthority( );
                ...
                internal DataTable GetTable(DataTable table, string fund)
                {
                    try
                    {
                        return table.AsEnumerable( ).Where(p => p.Field<string>("Fund").Equals(fund))
                ...
                internal Tuple<DataTable, PRC[], decimal, int> GetData(DataTable table, string filter)
                {
                    try
                    {
                ...
                private Dictionary<string, decimal> GetData(DataTable table, string[] filters, string column)
                {
                    try
                    {
                ...
                internal ChartControl GetChart(ChartControl chart, Dictionary<string, decimal> a)
                {
                ...
                internal ChartControl GetChart(ChartControl chart, Dictionary<string, decimal> a, Dictionary<string, decimal> b)
                {
                    var aseries = new ChartSeries("BOC", ChartSeriesType.Column);
                    foreach (KeyValuePair<string, decimal> kvp in a)
                    {
