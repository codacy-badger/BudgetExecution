...
namespace BudgetExecution
{
    ...
    public class SqlSvrQuery : Query
    {
        // CONSTRUCTORS
        public SqlSvrQuery()
        {
        ...
        public SqlSvrQuery(Source source) : base(source, Provider.SqlSvr)
        {
            Source = source;
            TableName = source.ToString();
            SelectStatement = $"SELECT * FROM {source.ToString()}";
            Connection = new SqlConnection(@"Data Source=C:\Users\terry\Documents\Visual Studio 2017\Projects\BudgetExecution\Ninja\database\SqlCe\R6.mdf");
        ...
        public SqlSvrQuery(Source source, Dictionary<string, object> param) : base(source, Provider.SqlSvr, param)
        {
            Source = source;
            Provider = Provider.SqlSvr;
            TableName = source.ToString();
            Parameter = param;
            SelectStatement = GetSqlStatement();
            Connection = new SqlConnection(@"data source=C:\Users\terry\Documents\Visual Studio 2017\Projects\Budget\database\SqlCe\R6.mdf");
            SelectCommand = new SqlCommand(SelectStatement, Connection);
            Adapter = new SqlDataAdapter(SelectCommand);
            CommandBuilder = GetCommandBuilder(Adapter);
            InsertCommand = CommandBuilder.GetInsertCommand();
